...
| Authoring or delivery tool | `Evidence validator` | Binds classified claims to approved snapshot files and preserves structured material uncertainty | Implemented in release `0.1.0` |
...
| Authoring or delivery tool | `validate.mjs` | Selects a bundled contract, validates generated bytes locally, filters safe issues by severity, formats JSON or text, and returns one of five statuses | Implemented in release `0.1.0` |
| Authoring or delivery tool | `generate-identities.mjs` | Derives prose-independent package and entity identifiers with collision stops | Implemented in release `0.1.0` |
| Authoring or delivery tool | `finalize-generation.mjs` | Records reproducible non-reasoning generation metadata, package checksum, and validation result | Implemented in release `0.1.0` |
| Authoring or delivery tool | `verify-ecosystem-analysis.mjs` | Verifies eleven C# or Angular evidence categories, local source paths, unresolved dynamic behavior, and cited user-to-API flow | Implemented in release `0.1.0` |
...
artifact retrieval, dependency-free package validation, local source-scope
preflight, evidence checks, stable identity generation, safe generation
receipts, and source-resolved C# and Angular profiles. Managed generation and a
signed distribution channel remain outside the implemented boundary.
