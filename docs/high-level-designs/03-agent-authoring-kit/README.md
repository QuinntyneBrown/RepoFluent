...
| --- | --- | --- | --- |
| `RepoFluent.Api` | `AuthoringKitDownloadEndpoint` | Lists compatible kit releases and returns signed download references | Target platform |
| Authoring or delivery tool | `Agent Guidance Bundle` | Constrains scope, provenance, uncertainty, secrets, and identifier behavior | Target artifact |
...
| Authoring or delivery tool | `RepoFluent.ContractCli` | Validates generated bytes before they leave the approved environment | Target executable |
...
The requirements and detailed designs define the bundle. The repository does not yet contain a released authoring kit or `RepoFluent.ContractCli` executable.
